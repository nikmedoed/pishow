...
class DeviceQueueManager:
    ...
    def list_collections(self):
        collections, labels = scan_collections(
            self.media_dict.media_dir,
            self.media_dict.background_suffix or "",
            self.media_dict.uploaded_media_raw,
        )
        # Detect mismatches to trigger a resync before next queue build.
        total_scanned = sum(c.files_count for c in collections)
        cached_total = len(self.media_dict)
        if total_scanned != cached_total:
            self.media_dict.mark_dirty()
        else:
            # Also compare per-collection counts for drift.
            for c in collections:
                cached_count = len(self.media_dict.collections_index.get(c.id, ()))
                if cached_count != c.files_count:
                    self.media_dict.mark_dirty()
                    break
