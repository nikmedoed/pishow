...
class MediaDict(dict):
    ...
    def keys_for_collections(self, collection_ids):
        if (not self.synced) or self.need_resync:
            self.sync_files()
    ...
    def mark_dirty(self):
        """Mark indexes stale so the next queue build will resync."""
        self.need_resync = True

    def ensure_metadata(self, key: str):
        """
        Ensure media exists and has minimal metadata.
        Returns MediaFile or None if missing.
        """
        media = self.get(key)
        if media is None:
            return None
